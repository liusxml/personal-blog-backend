package com.blog.file.infrastructure.config;
 ...
 * <li><strong>监控集成</strong>：注入 Micrometer {@link MeterRegistry} 打标签，Actuator
 * 可监控 Bitiful 请求指标。</li>
 * <li><strong>微服务 ready</strong>：整个类可直接复制到 file-server 模块，无需修改。</li>
 * <li><strong>工具类规范</strong>：Guava 不可变集合 + Lombok @RequiredArgsConstructor
 * 减少样板代码。</li>
 * </ul>
 * </p>
 *
 ...
 * <strong>兼容性：</strong>
 * <ul>
 * <li>Java 21 + Spring Boot 3.5.7</li>
 * <li>AWS SDK V2 2.30+</li>
...
public class BitifulConfig {
     ...
     * 绑定
     * <code>application.yaml</code> 中的 <code>oss.bitiful.*</code> 配置。
     ...
     * </p>
     * <ul>
     * <li><strong>endpointOverride</strong>：自定义 Bitiful endpoint
     * <code>https://s3.bitiful.net</code></li>
     * <li><strong>pathStyleAccessEnabled(true)</strong>：Bitiful 必须使用 path-style
     * 访问（bucket 在路径中）</li>
     ...
     *
     * <p>
     * <strong>监控集成</strong>：通过 Micrometer 打标签
     * <code>storage.provider=bitiful</code>，可在
    ...
    public S3Client bitifulS3Client(MeterRegistry meterRegistry) {
        ...
        // 可在 Actuator
        // 查看：s3_client_requests_total{storage.provider="bitiful",bucket="blog-files"}
        ...
        return S3Presigner.builder()
                ...
                .credentialsProvider(StaticCredentialsProvider.create(
                        AwsBasicCredentials.create(
                                properties.getAccessKey(),
                                properties.getSecretKey())))
                // CRITICAL: 必须配置为path-style，否则生成virtual-hosted-style URL导致连接失败
                // 正确: https://s3.bitiful.net/blog-module-file/key.jpg
                // 错误: https://blog-module-file.s4.bitiful.com/key.jpg （DNS不存在）
                .serviceConfiguration(S3Configuration.builder()
                        .pathStyleAccessEnabled(true)
                        .build())
                .build();
