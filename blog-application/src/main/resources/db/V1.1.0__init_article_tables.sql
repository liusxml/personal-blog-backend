...
CREATE TABLE IF NOT EXISTS `art_article` (
    ...
    -- ========== 高级特性 ==========
    `password` VARCHAR(100) COMMENT '访问密码（加密存储，可选）',
    `original_url` VARCHAR(500) COMMENT '原文链接（转载/翻译时填写）',
    `publish_time` DATETIME COMMENT '发布时间（首次发布时间）',
    `toc_json` TEXT COMMENT '目录结构（JSON格式，自动生成）',
    
    -- ========== 向量搜索（⭐ MySQL 9.4 新特性）==========
    `embedding` VECTOR(1536) COMMENT '文章内容向量（用于语义搜索和推荐）',
    ...
    -- ========== 索引设计 ==========
    PRIMARY KEY (`id`),
    KEY `idx_author_status` (`author_id`, `status`, `is_deleted`),
    KEY `idx_category` (`category_id`),
    KEY `idx_publish_time` (`publish_time`),
    KEY `idx_status_top` (`status`, `is_top`, `is_deleted`),
    KEY `idx_create_time` (`create_time`),
    FULLTEXT KEY `idx_fulltext` (`title`, `summary`) WITH PARSER ngram
...
CREATE TABLE IF NOT EXISTS `art_article_stats` (
    ...
    `article_id` BIGINT NOT NULL COMMENT '文章ID（外键 art_article.id）',
...
CREATE TABLE IF NOT EXISTS `art_category` (
    ...
    `slug` VARCHAR(50) NOT NULL COMMENT 'URL友好标识（用于生成链接）',
    `icon` VARCHAR(100) COMMENT '分类图标（图标名称或URL）',
    `description` VARCHAR(200) COMMENT '分类描述',
    
    -- ========== 层级关系 ==========
    `parent_id` BIGINT COMMENT '父分类ID（NULL表示顶级分类）',
    `path` VARCHAR(500) COMMENT '分类路径（便于查询子分类，格式: /1/2/3）',
...
CREATE TABLE IF NOT EXISTS `art_tag` (
    ...
    `color` VARCHAR(20) COMMENT '标签颜色（HEX格式，如 #3B82F6）',
...
CREATE TABLE IF NOT EXISTS `art_article_tag` (
    ...
    -- ========== 关系数据 ==========
    `article_id` BIGINT NOT NULL COMMENT '文章ID（外键 art_article.id）',
    `tag_id` BIGINT NOT NULL COMMENT '标签ID（外键 art_tag.id）',
...
INSERT IGNORE INTO `art_category` (`id`, `name`, `slug`, `description`, `parent_id`, `path`, `sort_order`, `create_time`)
VALUES 
...
INSERT IGNORE INTO `art_tag` (`id`, `name`, `slug`, `color`, `create_time`)
VALUES
